...
def make_plot(frame, show = False):
    ...
    radial_velocity = (fromfile("gasvrad%d.dat" % frame).reshape(num_rad, num_theta))
    azimuthal_velocity = (fromfile("gasvtheta%d.dat" % frame).reshape(num_rad, num_theta))
    if center:
        if taper_time < 10.1:
            shift_c = az.get_azimuthal_peak(gas_density, fargo_par)
        else:
            threshold = util.get_threshold(size) * 1.5
            shift_c = az.get_azimuthal_center(gas_density, fargo_par, threshold = threshold * surface_density_zero)
    ...
    if version is None:
        ...
        save_fn = "%s/v%04d_cflTimestep_%04d.png" % (save_directory, version, frame)
