...
def new_argument_parser(description = "Plot gas density maps."):
    ...
    # Files
    parser.add_argument('--dir', dest = "save_directory", default = "verticalTimeAveragedAlphaProfile",
...
def get_data(directory = "./", num_z = num_z):
    ...
    for frame in frame_range:
        ...
        # Sum over time
        total_averaged_reynolds_stress_rad_theta += averaged_reynolds_stress_rad_theta

    # Time Average
    total_averaged_reynolds_stress_rad_theta /= len(frame_range)
    ...
    omega_gradient = -1.5 * np.power(r_select, -2.5)
    scale_height_function = scale_height * r_select
    ...
    coefficient = -1.0 * r_select * omega_gradient * density * sound_speed_function * np.power(scale_height_function, 2.0)
...
def make_plot(show = False):
    ...
    result, = plot.plot(x, y, linewidth = linewidth, c = "b", label = cwd, zorder = 90)
    ...
    if version is None:
        save_fn = "%s/%s_timeAveragedAlphaVerticalProfile_%04d-%04d-%04d.png" % (save_directory, directory_name, args.frames[0], args.frames[1], args.frames[2])
    else:
        save_fn = "%s/v%04d_%s_timeAveragedAlphaVerticalProfile_%04d-%04d-%04d.png" % (save_directory, version, directory_name, args.frames[0], args.frames[1], args.frames[2])
