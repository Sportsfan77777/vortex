...
def make_plot(frame, show = False):
    ...
    # Shift
    if center:
       normalized_density, midplane_vz, shift_c = shift_density(normalized_density, midplane_vz, fargo_par, reference_density = normalized_density)

    ### Plot ###
    x = rad
    y = theta * (180.0 / np.pi)
    result = ax.pcolormesh(x, y, np.transpose(midplane_vz), cmap = cmap)
