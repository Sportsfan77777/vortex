...
def get_excess_mass(args_here):
    ...
    field = "dens"
    if mpi:
      spatial_density = Fields("./", 'dust1', frame).get_field(field).reshape(num_z, num_rad, num_theta)
      #gas_density = Fields("./", 'gas', frame).get_field(field).reshape(num_z, num_rad, num_theta)
    else:
      spatial_density = fromfile("dust1dens%d.dat" % frame).reshape(num_z, num_rad, num_theta)
