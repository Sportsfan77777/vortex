...
def make_plot(frames, show = False):
    ...
    def add_to_plot(i):
        ...
        if i == 3:
            ...
            text4 = r"%d$\times$%d $\times$ %d" % (num_rad, num_theta, num_z)
    ...
    if version is None:
        save_fn = "%s/%s_dustDensityMap_%04d-%04d-%04d-%04d.png" % (save_directory, directory_name, frames[0], frames[1], frames[2], frames[3])
    else:
        save_fn = "%s/v%04d_%s_dustDensityMap_%%04d-%04d-%04d-%04d.png" % (save_directory, directory_name, frames[0], frames[1], frames[2], frames[3])
