...
void ChangeArch() {
  ...
  AccreteOntoPlanets = AccreteOntoPlanets_cpu;
  ...
  while (fgets(s, MAXLINELENGTH-1, func_arch) != NULL) {
	...
	if(strval[0] == 'g'){
      ...
      if (strcmp(name, "accreteontoplanets") == 0) {
	if(strval[0] == 'g'){
          AccreteOntoPlanets = AccreteOntoPlanets_gpu;
	  printf("AccreteOntoPlanets runs on the GPU\n");
	}
      }
