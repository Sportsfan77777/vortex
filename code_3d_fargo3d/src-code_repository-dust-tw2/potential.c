...
void compute_potential(real dt) {
  ...
  if (alreadycalculated != Timestepcount){ //For multifluid purposes...
    ...
    DevMemcpyH2D(Sys->accreted_mass_gpu, Sys->accreted_mass_cpu, sizeof(real)*(Sys->nb+1));
#endif
...
//<USER_DEFINED>
  ...
  if (PhysicalTime < 2.0 * M_PI * PLANETSTART)
    planetmass_taper = 0.0;
  else if (MASSTAPER == 0.0)
    planetmass_taper = 1.0;
  else
    planetmass_taper = ((PhysicalTime - 2.0 * M_PI * PLANETSTART) >= (2.0 * M_PI * MASSTAPER) ? 1.0 : .5*(1.0-cos(M_PI*(PhysicalTime - 2.0 * M_PI * PLANETSTART)/(2.0 * M_PI * MASSTAPER))));
//<\USER_DEFINED>
...
//<#>
	...
	for(n=0; n<nb; n++) {
	  mp = mplanet[n]*taper + accreted_mass[n] + 0.00000001;
