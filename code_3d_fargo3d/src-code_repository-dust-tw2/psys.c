...
PlanetarySystem *AllocPlanetSystem(int nb) {
  ...
  status = DevMalloc(&(sys->acc_gpu),(sizeof(real)*(nb+1)));
...
PlanetarySystem *InitPlanetarySystem (char *filename) {
  ...
  while (fgets(s, 510, input) != NULL) {
    ...
    if (isalpha(s[0])) {
      s1 = s + strlen(nm);
#ifdef FLOAT
      sscanf(s1 + strspn(s1, "\t :=>_"), "%f %f %f %s %s", &dist, &mass, &accret, test1, test2, accret_mass);
#else
      sscanf(s1 + strspn(s1, "\t :=>_"), "%lf %lf %lf %s %s", &dist, &mass, &accret, test1, test2, accret_mass);
#endif
