...
def new_argument_parser(description = "Plot gas density maps."):
    ...
    # Files
    parser.add_argument('--dir', dest = "save_directory", default = "3DstackVerticalShearMaps",
                         help = 'save directory (default: 3DstackVerticalShearMaps)')
...
def make_plot(z_level, show = False):
    ...
    density_z = density[num_z / 2 + z_level, :, :]
    ...
    if center:
        domega_dz_z, shift_c = shift_density(domega_dz_z, fargo_par, reference_density = normalized_density)
    ...
    # Save, Show, and Close
    if version is None:
        save_fn = "%s/verticalShearMap_%04d-z%04d.png" % (save_directory, frame, z_level)
    else:
        save_fn = "%s/v%04d_verticalShearMap_%04d-z%04d.png" % (save_directory, version, frame, z_level)
