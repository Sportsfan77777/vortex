...
def new_argument_parser(description = "Plot gas density maps."):
    ...
    # Files
    parser.add_argument('--dir', dest = "save_directory", default = "kineticEnergy",
                         help = 'save directory (default: kineticEnergy)')
...
def get_kinetic_energy(args_here):
    ...
    def get_data(directory = "./", num_z = num_z):
        if mpi:
          ...
          vtheta = Fields(directory, 'gas', frame).get_field("vx").reshape(num_z, num_rad, num_theta)
          vz = Fields(directory, 'gas', frame).get_field("vz").reshape(num_z, num_rad, num_theta)
        else:
...
def make_plot(show = False):
    ...
    if version is None:
        ...
        save_fn = "%s/v%04d_%s_kineticEnergy.png" % (save_directory, version, directory_name)
