...
def make_plot(show = False):
    ...
    png = "png"; pdf = "pdf"
    if version is None:
        save_fn = "%s/azimuthalIntensityEvolution_%s.%s" % (save_directory, frame_str, png)
        pdf_save_fn = "%s/azimuthalIntensityEvolution_%s.%s" % (save_directory, frame_str, pdf)
    else:
        save_fn = "%s/v%04d_azimuthalIntensityEvolution_%s.%s" % (save_directory, version, frame_str, png)
        pdf_save_fn = "%s/v%04d_azimuthalIntensityEvolution_%s.%s" % (save_directory, version, frame_str, pdf)
