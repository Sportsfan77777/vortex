...
def new_argument_parser(description = "Plot dust density maps."):
    ...
    # Files
    parser.add_argument('--dir', dest = "save_directory", default = "dustDensityMapComparisons",
                         help = 'save directory (default: dustDensityMapComparisons)')

    parser.add_argument('--dir1', dest = "dir1", default = ".",
                         help = 'save directory (default: .)')
    parser.add_argument('--dir2', dest = "dir2", default = "../../taper750-weak_dust_h-no_sfta/cm-size/",
                         help = 'save directory (default: ../../taper750-weak_dust_h-no_sfta/cm-size/)')
...
def make_plot(frame, show = False):
    ...
    # Data
    gas_density1 = (fromfile("%s/gasdens%d.dat" % (dir1, frame)).reshape(num_rad, num_theta)) / gas_surface_density_zero
    density1 = (fromfile("%s/gasddens%d.dat" % (dir1, frame)).reshape(num_rad, num_theta))
    normalized_density1 = density1 / dust_surface_density_zero

    gas_density2 = (fromfile("%s/gasdens%d.dat" % (dir2, frame)).reshape(num_rad, num_theta)) / gas_surface_density_zero
    density2 = (fromfile("%s/gasddens%d.dat" % (dir2, frame)).reshape(num_rad, num_theta))
    normalized_density2 = density2 / dust_surface_density_zero
    ...
    #   normalized_density, shift_c = shift_density(normalized_density, fargo_par, option = center[3:], reference_density = cm_dust_density, frame = frame)
       ...
       normalized_density1, shift_c1 = shift_density(normalized_density1, fargo_par, option = center, reference_density = gas_density1, frame = frame)
       normalized_density2, shift_c2 = shift_density(normalized_density2, fargo_par, option = center, reference_density = gas_density2, frame = frame)
    else:
       normalized_density1, shift_c1 = shift_density(normalized_density1, fargo_par, option = center, frame = frame)
       normalized_density2, shift_c2 = shift_density(normalized_density2, fargo_par, option = center, frame = frame)
    ...
    if use_contours:
        ...
        plot.contour(x, y, np.transpose(gas_density2), levels = levels, origin = 'upper', linewidths = 1, colors = colors)
