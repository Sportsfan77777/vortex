...
save_directory = "bothAveragedThetaVelocities"
...
def make_plot(frame, show = False):
    ...
    def choose_axis(i, axis):
        ...
        plot.plot(x, averaged_vtheta_dust, linewidth = linewidth)
        ...
        ### Plot ###
        plot.plot(x, averaged_vtheta_dust - averaged_vtheta, linewidth = linewidth)

        # Annotate
        this_title = readTitle()
        plot.xlabel(xlabel, fontsize = fontsize)
        plot.ylabel(r"$\eta = V_{\phi, \mathrm{dust}} - V_{\phi, \mathrm{gas}}$", fontsize = fontsize)
        plot.title("Orbit %d: %s" % (orbit, this_title), fontsize = fontsize + 1)

        # Save and Close
        plot.savefig("%s/%sboth_avg_theta_velocities_%04d.png" % (save_directory, prefix, i), bbox_inches = 'tight', dpi = my_dpi)
        if show:
            plot.show()
