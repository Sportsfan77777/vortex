...
def get_data(frame):
    ...
    azimuthal_gas_profiles = [density[azimuthal_index, :] for azimuthal_index in azimuthal_indices]
    azimuthal_dust_profiles = [dust_density[azimuthal_index, :] for azimuthal_index in azimuthal_indices]

    return azimuthal_radii, azimuthal_gas_profiles, azimuthal_dust_profiles
...
def make_plot(frame, azimuthal_radii, azimuthal_gas_profiles, azimuthal_dust_profiles, show = False):
    # Orbit Number
    ...
    ### Plot ###
    for i, (radius, azimuthal_gas_profile, azimuthal_dust_profile) in enumerate(zip(azimuthal_radii, azimuthal_gas_profiles, azimuthal_dust_profiles)):
        plot.plot(theta, azimuthal_gas_profile, linewidth = linewidth, alpha = alpha, c = color[i], label = "%.3f" % radius)
        plot.plot(theta, 50 * np.array(azimuthal_dust_profile), linewidth = linewidth, c = color[i], alpha = alpha)
...
if len(sys.argv) > 1:
    ...
    if frame_number == -1:
        ...
        for i in sample:
            azimuthal_radii, azimuthal_gas_profiles, azimuthal_dust_profiles = get_data(i)
