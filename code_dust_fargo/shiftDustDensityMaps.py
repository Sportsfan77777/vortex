...
def make_plot(frame, show = False):
    ...
    def choose_axis(i, axis):
        ...
        plot.title("Shifted %s-size Dust Density Map at Orbit %d\n%s" % (size, orbit, this_title), fontsize = fontsize + 1)

        # Save and Close
        plot.savefig("%s/%sshiftedDustDensityMap%04d_%s.png" % (save_directory, prefix, i, size), bbox_inches = 'tight', dpi = my_dpi)
        if show:
            plot.show()
