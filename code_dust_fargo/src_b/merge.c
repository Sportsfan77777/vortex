     ...
     int nb;
{
  ...
  for (j = 0; j < 7+(AdvecteLabel == YES); j++) {
    switch (j) {
    case 0: strcpy (radix, "ddens");
      break;
    case 1: strcpy (radix, "dvrad");
      break;
    case 2: strcpy (radix, "dvtheta");
      break;
    case 3: strcpy (radix, "TStop");
      break;
    case 4: strcpy (radix, "Diag1");
      break;
    case 5: strcpy (radix, "Diag2");
      break;
    case 6: strcpy (radix, "Diag3");
      break;
    case 7: strcpy (radix, "label");
      break;
    }
    for (i = 1; i < CPU_Number; i++) {
      sprintf (command, "cd %s; cat gas%s%d.dat.%05d >> gas%s%d.dat",\
