...
save_directory = "bothSquarePolarGasDensityMaps"
...
def make_plot(frame, show = False):
    ...
    def choose_axis(i, axis):
        ...
        cbar.set_label(r"Normalized Gas Surface Density", fontsize = fontsize + 2, rotation = 270, labelpad = 20)
        ...
        # Annotate
        title1 = r"$m_p = %d$ $M_{Jup}$, $\alpha_{disk} = %d \times 10^{%d}$, $T_{growth} = %d$ $\rm{orbits}$" % (int(planet_mass / 0.001), 3, int(np.log(viscosity) / np.log(10)) + 2, taper_time)
        ...
        cbar.set_label(r"Normalized Dust Surface Density", fontsize = fontsize + 2, rotation = 270, labelpad = 20)
        ...
        # Save and Close
        plot.savefig("%s/%sbothSquareDensityMap_%04d.png" % (save_directory, prefix, i), bbox_inches = 'tight', dpi = my_dpi)
        if show:
            plot.show()
