...
### Choose Sample ###
sample_name = "first100by20"
sample_range = np.linspace(0, 100, 6)
...
def add_to_plot(frame, choice = "normal", show = False):
    ...
    def choose_axis(i, axis):
        ...
        plot.ylabel(r"Azimuthally Averaged $V_{\phi, \mathrm{gas}}$", fontsize = fontsize)
        ...
        ### Plot ###
        plot.plot(x, averaged_vtheta_dust, linewidth = linewidth, label = "%d" % frame)

        # Annotate
        this_title = readTitle()
        plot.xlabel(xlabel, fontsize = fontsize)
        plot.ylabel(r"Azimuthally Averaged $V_{\phi, \mathrm{dust}}$", fontsize = fontsize)
        plot.title("%s" % this_title, fontsize = fontsize + 1)
...
def make_plot(prefix = "", show = False):
    ...
    # Save and Close
    plot.savefig("%s/%savg_theta_velocity_%s.png" % (directory, prefix, sample_name), bbox_inches = 'tight', dpi = my_dpi)
    if show:
        plot.show()
