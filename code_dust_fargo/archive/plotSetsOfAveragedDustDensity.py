...
name = "averagedDustDensities%d.p"
ids = [5524, 6420]
labels = ["both", "gas"]
...
def make_plot(i, frame, show = False):
    ...
    def choose_axis(i, frame, axis):
        ...
        plot.ylabel("Azimuthally-Averaged\n Normalized Dust Density", fontsize = fontsize)
        plot.title(r"$t = %d$ $\rm{orbits}$" % (orbit), fontsize = fontsize + 1)

        plot.legend(loc = "upper right")

        # Save and Close
        plot.savefig("%s/%savg_dust_density_%04d.png" % (save_directory, prefix, frame), bbox_inches = 'tight', dpi = my_dpi)
        if show:
            plot.show()
