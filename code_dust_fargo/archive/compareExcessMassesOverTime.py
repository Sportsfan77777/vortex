...
# Plot Curves
for taper in tapers:
    frame_range = pickle.load(open("excess_mass_frames_taper%s.p" % taper, "rb"))
    mass_over_time = pickle.load(open("excess_mass_values_taper%s.p" % taper, "rb"))

    # Curve
    plot.plot(frame_range, mass_over_time, linewidth = linewidth, label = r"$T_{growth}=$" + "%s" % taper)
