...
def get_data():
    ...
    azimuthal_profiles = [intensity[azimuthal_index, :] for azimuthal_index in azimuthal_indices]
...
def make_plot(frame, azimuthal_radii, azimuthal_profiles, show = False):
    ...
    ### Plot ###
    for radius_i, azimuthal_profile in zip(azimuthal_radii, azimuthal_profiles):
        plot.plot(theta, azimuthal_profile, linewidth = linewidth, alpha = alpha, label = "%.3f" % radius_i)
    ...
    legend = plot.legend(loc = "upper right", bbox_to_anchor = (1.28, 1.0)) # outside of plot)

    # Save and Close
    plot.savefig("%s/azimuthal_intensity_%04d_at%d_r%02d_h%d_n%d.png" % (directory, frame, wavelength, int(10 * chosen_radius), int(num_scale_heights), int(num_profiles)), bbox_inches = 'tight', bbox_extra_artists = (legend, ), dpi = my_dpi)
    if show:
        plot.show()
