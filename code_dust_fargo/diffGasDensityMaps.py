...
def new_argument_parser(description = "Plot dust density maps."):
    ...
    # Sizes
    parser.add_argument('--sizes', dest = "sizes", type = int, nargs = 2, default = ["cm", "hcm"],
                         help = 'select two grain sizes to compare (default: cm, hcm)')

    # Files
    parser.add_argument('--dir', dest = "save_directory", default = "diffDensityMaps_%s-%s",
                         help = 'save directory (default: diffDensityMaps_%s-%s)')
    ...
    parser.add_argument('--cmap', dest = "cmap", default = "PuOr",
                         help = 'color map that is ideally diverging (default: PuOr)')
    parser.add_argument('--cmax', dest = "cmax", type = float, default = 0.5,
                         help = 'maximum diff density in colorbar (default: [-0.5, 0.5])')
...
fargo_par = util.get_pickled_parameters(directory = "../%s-size/" % args.sizes[0])
...
# Files
save_directory = args.save_directory % (sizes[0], sizes[1])
...
def make_plot(frame, show = False):
    ...
    # Save, Show, and Close
    if version is None:
        save_fn = "%s/diffDensityMap_%04d.png" % (save_directory, frame)
    else:
        save_fn = "%s/v%04d_diffDensityMap_%04d.png" % (save_directory, version, frame)
