"""
plot 3-D mass accretion rate slices of 3-D simulations
\dot{M} = \rho * v_R * R

Options:

Usage:
python plotRadialVelocitySlices.py --frame 500 -z 0 (or -t, -r)
python plotRadialVelocitySlices.py --frame 500 -z 0 (or -t, -r) --a 1.0 -b 2.0 # center on vortex
"""
...
save_directory = "massAccretionRateSlices"
...
if ((pluto_par["X1-grid"])[3] == "l+"):
    # log grid
    rad = np.logspace(np.log10(float((pluto_par["X1-grid"])[1])), np.log10(float((pluto_par["X1-grid"])[4])), num_rad)
else:
    # uniform grid
...
def make_plot(frame, show = False):
    ...
    density_normalization = surface_density_zero / (np.sqrt(2.0 * np.pi) * scale_height)
    normalized_density = density / density_normalization
    ...
    if o.r_slice is not None:
        ...
        mass_accretion_rate_slice = mass_accretion_rate[:, :, this_slice_i].T
        ...
        mass_accretion_rate_slice = mass_accretion_rate[this_slice_i, :, :]
    ...
    print "Max:", np.max(mass_accretion_rate), np.max(mass_accretion_rate_slice) # Diagnostic
    print "Min:", np.min(mass_accretion_rate), np.min(mass_accretion_rate_slice) # Diagnostic

    result = ax.pcolormesh(xs, ys, mass_accretion_rate_slice, cmap = cmap)
    ...
    if o.clim_b is not None:
        ...
        result.set_clim(np.min(mass_accretion_rate_slice), 0.0)
    ...
    plot.title("Mass Accretion Rate Slice (%s = %s) at Orbit %d" % (slice_choice, this_slice, orbit), fontsize = fontsize + 1)

    # Save and Close
    plot.savefig("%s/%s_massAccretionRateSlice_%s%04d.png" % (save_directory, o.prefix, suffix, frame), bbox_inches = 'tight', dpi = my_dpi)
    if show:
        plot.show()
...
def new_option_parser():
  ...
  parser.add_option("-p", 
                    ...
                    help="clim min")
  parser.add_option("-q", 
                    dest="clim_b", type = "float", default = None,
                    help="clim max")
