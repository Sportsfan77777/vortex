"""
plot 3-D radial velocity slices of 3-D simulations
...
save_directory = "thetaVelocitySlices"
...
def make_plot(frame, show = False):
    ...
    normalized_theta_velocity = theta_velocity / normalization
    ...
    if o.r_slice is not None:
        ...
        theta_velocity_slice = normalized_theta_velocity[:, :, this_slice_i].T
        ...
        theta_velocity_slice = normalized_theta_velocity[this_slice_i, :, :]
    ...
    print "Max:", np.max(normalized_theta_velocity), np.max(theta_velocity_slice) # Diagnostic
    print "Min:", np.min(normalized_theta_velocity), np.min(theta_velocity_slice) # Diagnostic

    result = ax.pcolormesh(xs, ys, theta_velocity_slice, cmap = cmap)
    ...
    plot.title("Theta Velocity Slice (%s = %s) at Orbit %d" % (slice_choice, this_slice, orbit), fontsize = fontsize + 1)

    # Save and Close
    plot.savefig("%s/%s_thetaVelocitySlice_%s%04d.png" % (save_directory, o.prefix, suffix, frame), bbox_inches = 'tight', dpi = my_dpi)
    if show:
        plot.show()
