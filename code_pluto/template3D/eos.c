  ...
  #elif EOS == ISOTHERMAL
  {
    ...
    if (g_dir == IDIR) {
      ...
      for (i = beg; i <= end; i++){
         ...
         R = x1[i]*sin(x2[j]);
        #endif
        //cs2[i] = g_isoSoundSpeed*g_isoSoundSpeed/R; --- original
        cs2[i] = pow(soundSpeed(R), 2);
      }
      ...
      for (j = beg; j <= end; j++) {
         ...
         R = x1[i]*sin(x2[j]);
        #endif
        //cs2[j] = g_isoSoundSpeed*g_isoSoundSpeed/R; --- original
        cs2[j] = pow(soundSpeed(R), 2);
      }
    }else if (g_dir == KDIR){
      double R;
      ...
      for (k = beg; k <= end; k++){
         ...
         R = x1[i]*sin(x2[j]);
        #endif
        //cs2[k] = g_isoSoundSpeed*g_isoSoundSpeed/R; --- original
        cs2[k] = pow(soundSpeed(R), 2);
      }
    }
  }
  #else
