...
void FixedBoundary(const Data *d, RBox *box, int side, Grid *grid) {
  // Applies a fixed boundary at the specified boundary (side)
  int   i, j, k, nv;
...
/// Evanescent Boundary (internal) ///

void EvanescentBoundary(const Data *d, RBox *box, int side, Grid *grid) {
  /// Applies evanescent boundary conditions in the specified damping zone
  int   i, j, k, nv;
  double *x1, *x2, *x3, R, th, z, OmegaK, v[256];

  // Add if side == 0? (or put that before the call in init.c?)
 ...
 *********************************************************************** */
{
  ...
  /// Evanescent Boundary Conditions ///
