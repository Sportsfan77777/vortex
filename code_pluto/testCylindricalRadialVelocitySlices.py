...
save_directory = "deltaRadialVelocitySlices"
...
def make_plot(frame, show = False):
    ...
    if o.r_slice is not None:
        ...
        radial_velocity_slice = delta_radial_velocity[:, :, this_slice_i].T
        ...
        radial_velocity_slice = delta_radial_velocity[this_slice_i, :, :]
    ...
    plot.title("Delta Cylindrical Radial Velocity Slice (%s = %s) at Orbit %d" % (slice_choice, this_slice, orbit), fontsize = fontsize + 1)

    # Save and Close
    plot.savefig("%s/%s_deltaCylindricalRadialVelocitySlice_%s%04d.png" % (save_directory, o.prefix, suffix, frame), bbox_inches = 'tight', dpi = my_dpi)
    if show:
        plot.show()
