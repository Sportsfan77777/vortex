     ...
     PolarGrid *Rho;
{
  ...
  if ( SG_initcounter == 0 ) {
	...
	SGP_Kr[i] += ( 1.0 + SGP_eps*SGP_eps - cos(theta)*exp(-u) ) *	\
	  pow(SGP_eps*SGP_eps*exp(u) + 2.0*( cosh(u) - cos(theta) ),-1.5);
      }
      SGP_Kr[i] *= SGP_tstep;
    }
  ...
  integrale = 0.0;
  for ( j = 0; j < NSEC; j++) {
    theta = 2.0*M_PI*(real)j/(real)NSEC;
    num = 1.0 + SGP_eps*SGP_eps - cos(theta);
    den = pow(2.0*(1.0-cos(theta)) + SGP_eps*SGP_eps,1.5);
    integrale += num/den;
  }
  integrale *= SGP_tstep;
  for ( i = 0; i < GLOBALNRAD; i++ )
