...
def new_argument_parser(description = "Plot gas density maps."):
    ...
    # Files
    parser.add_argument('--dir', dest = "save_directory", default = "averagedScaleHeight",
                         help = 'save directory (default: averagedScaleHeight)')
...
def make_plot(frame, show = False):
    ...
    if merge > 0:
        ...
        field = "dens"
        density = Fields("./", 'gas', frame).get_field('dens').reshape(num_rad, num_theta)
        energy = Fields("./", 'gas', frame).get_field('energy').reshape(num_rad, num_theta)
    else:
    ...
    if args.max_y is None:
        ...
        highest_y = 1.1 * max(y[x_min_i : x_max_i])
        lowest_y = 1.1 * min(y[x_min_i : x_max_i])

        if highest_y - scale_height > scale_height - lowest_y:
            delta_y = highest_y - scale_height
        else:
            delta_y = scale_height - lowest_y

        max_y = scale_height + 1.1 * delta_y
        min_y = scale_height - 1.1 * delta_y
    else:
        max_y = scale_height + args.max_y
        min_y = scale_height - args.max_y
    ...
    # Save, Show, and Close
    if version is None:
        save_fn = "%s/averagedScaleHeight_%04d.png" % (save_directory, frame)
    else:
        save_fn = "%s/v%04d_averagedScaleHeight_%04d.png" % (save_directory, version, frame)
