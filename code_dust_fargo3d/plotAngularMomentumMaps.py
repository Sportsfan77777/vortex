...
def new_argument_parser(description = "Plot gas density maps."):
    ...
    # Files
    parser.add_argument('--dir', dest = "save_directory", default = "angularMomentumMaps",
                         help = 'save directory (default: angularMomentumMaps)')
    ...
    parser.add_argument('--cmax', dest = "cmax", type = float, default = 0.10,
                         help = 'maximum radial velocity in colorbar (default: 0.10)')
...
def make_plot(frame, show = False):
    ...
    x = rad
    y = theta * (180.0 / np.pi)
    result = ax.pcolormesh(x, y, np.transpose(angular_momentum), cmap = cmap)
    ...
    # Save, Show, and Close
    if version is None:
        save_fn = "%s/angularMomentumMap_%04d.png" % (save_directory, frame)
    else:
        save_fn = "%s/v%04d_angularMomentumMap_%04d.png" % (save_directory, version, frame)
