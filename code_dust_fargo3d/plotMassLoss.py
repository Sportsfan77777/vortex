...
def new_argument_parser(description = "Plot gas density maps."):
    ...
    # Files
    parser.add_argument('--dir', dest = "save_directory", default = "massLoss",
                         help = 'save directory (default: massLoss)')
...
def make_plot(frame, show = False):
    ...
    plot.plot(xref_d, yref_d, linewidth = linewidth, c = "grey", zorder = 2, label = "A&B Omega_p")
    plot.plot(xref_d, yref_d2, linewidth = linewidth, c = "brown", zorder = 2, label = "A&B Omega(r)")
    ...
    ax.set_ylabel(r"$\dot{M}$ [$\Sigma_0 R_\mathrm{p}^2 \Omega_\mathrm{p}$]", fontsize = fontsize)
    ...
    # Save, Show, and Close
    if version is None:
        save_fn = "%s/massLoss_%04d.png" % (save_directory, frame)
    else:
        save_fn = "%s/v%04d_massLoss_%04d.png" % (save_directory, version, frame)
