...
def make_plot(frame, show = False):
    ...
    if merge > 0:
        ...
        velocity = util.read_merged_data(frame, num_merged_cores, num_rad, num_theta, field = "vx", fluid = "dust%d" % dust_number)
    elif mpi:
        gas_density = Fields("./", 'gas', frame).get_field("dens").reshape(num_rad, num_theta)
        velocity = Fields("./", 'dust%d' % dust_number, frame).get_field("vx").reshape(num_rad, num_theta)
    else:
