...
def new_argument_parser(description = "Plot gas density maps."):
    ...
    # Files
    parser.add_argument('--dir', dest = "save_directory", default = "dustContrast",
                         help = 'save directory (default: dustContrast)')
...
def get_excess_mass(args_here):
    ...
    def helper(density):
        ...
        maximum = np.max(vortex_excess)
        minimum = np.min(vortex_excess)
...
def make_plot(show = False):
    ...
    if args.data:
        frame_range_data = pickle.load(open("%s/dust_contrast_frames.p" % args.data, "rb"))
        mass_over_time_data = pickle.load(open("%s/dust_contrast_frames.p" % args.data, "rb"))
    ...
    plot.ylabel("Dust Contrast", fontsize = fontsize)
    ...
    if version is None:
        save_fn = "%s/%s_dustContrast.png" % (save_directory, directory_name)
    else:
        save_fn = "%s/v%04d_%s_dustContrast.png" % (save_directory, version, directory_name)
