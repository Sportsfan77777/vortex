...
def new_argument_parser(description = "Plot gas density maps."):
    ...
    # Files
    parser.add_argument('--dir', dest = "save_directory", default = "meanAzimuthalProfileSeries",
                         help = 'save directory (default: meanAzimuthalProfileSeries)')
...
def make_plot(frame_range, show = False):
    ...
    if args.start is not None:
        ...
        plot.text(5, 0.12 * max_y, text_start, fontsize = fontsize - 4, color = 'black', horizontalalignment = 'left')
    if args.end is not None:
        text_end = r"$t_\mathrm{end}$  $=$ $%d$ $T_\mathrm{p}$" % args.end
        plot.text(5, 0.05 * max_y, text_end, fontsize = fontsize - 4, color = 'black', horizontalalignment = 'left')
