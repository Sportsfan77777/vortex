...
def new_argument_parser(description = "Plot gas density maps."):
    ...
    # Files
    parser.add_argument('--dir', dest = "save_directory", default = "dustAzimuthalVelocityMaps",
                         help = 'save directory (default: dustAzimuthalVelocityMaps)')
...
def make_plot(frame, show = False):
    ...
    azimuthal_gas_velocity = fromfile("gasvx%d.dat" % frame).reshape(num_rad, num_theta)
    radial_gas_velocity = fromfile("gasvy%d.dat" % frame).reshape(num_rad, num_theta)

    azimuthal_dust_velocity = fromfile("dust%dvx%d.dat" % (dust_number, frame)).reshape(num_rad, num_theta)
    radial_dust_velocity = fromfile("dust%dvy%d.dat" % (dust_number, frame)).reshape(num_rad, num_theta)
    ...
    azimuthal_gas_velocity -= keplerian_velocity[:, np.newaxis]
    azimuthal_dust_velocity -= keplerian_velocity[:, np.newaxis]

    ### Plot ###
    x = rad
    y = theta * (180.0 / np.pi)
    result = ax.pcolormesh(x, y, np.transpose(azimuthal_dust_velocity - azimuthal_gas_velocity), cmap = cmap)
