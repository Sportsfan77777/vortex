...
def get_extents(args_here):
    ...
    if normal:
        ...
        cutoff = -0.04
        left_i = rad_min_i + len(left_side) - az.my_searchsorted(left_side[::-1], cutoff) # at location of minimum
        right_i = rad_min_i + az.my_searchsorted(right_side, cutoff)
        front_i = theta_min_i + len(front_side) - az.my_searchsorted(front_side[::-1], cutoff)
        back_i = theta_min_i + az.my_searchsorted(back_side, cutoff)
        ...
        left_i = radial_center_i + len(left_side) - az.my_searchsorted(left_side[::-1], cutoff) # relative to center
        right_i = radial_center_i + az.my_searchsorted(right_side, cutoff)
        front_i = azimuthal_center_i + len(front_side) - az.my_searchsorted(front_side[::-1], cutoff)
        back_i = azimuthal_center_i + az.my_searchsorted(back_side, cutoff)
