...
def new_argument_parser(description = "Plot gas density maps."):
    ...
    # Files
    parser.add_argument('--dir', dest = "save_directory", default = "slipConditionMaps",
                         help = 'save directory (default: slipConditionMaps)')
...
def make_plot(frame, show = False):
    ...
    # Calculate Slip Condition
    upper_gas_density = np.roll(gas_density, 1, axis = -1)
    upper_velocity = np.roll(velocity, 1, axis = -1)
    ...
    x = rad
    y = theta * (180.0 / np.pi)
    result = ax.pcolormesh(x, y, np.transpose(slip_condition), cmap = cmap)
    ...
    # Save, Show, and Close
    if version is None:
        save_fn = "%s/slipConditionMap_%04d.png" % (save_directory, frame)
    else:
        save_fn = "%s/v%04d_slipConditionMap_%04d.png" % (save_directory, version, frame)
