...
def find_peak(averagedDensity):
    ...
    outer_disk_end = np.searchsorted(rad, 2.1) # look for max radial density before r = 2.1
...
def get_excess_mass(args_here):
    ...
    def helper(density):
        ...
        diff_density = density - averagedDensity[:, None]
...
pickle.dump(np.array(frame_range), open("difference_mass_frames.p", "wb"))
pickle.dump(np.array(mass_over_time), open("difference_mass_values.p", "wb"))
...
def make_plot(show = False):
    ...
    if args.data:
        frame_range_data = pickle.load(open("%s/difference_mass_frames.p" % args.data, "rb"))
        mass_over_time_data = pickle.load(open("%s/difference_mass_values.p" % args.data, "rb"))
    ...
    plot.ylabel("Difference Mass", fontsize = fontsize)
    ...
    # Save + Close
    plot.savefig("differenceMass.png")
    plot.show()
