...
def new_argument_parser(description = "Plot gas density maps."):
    ...
    # Files
    parser.add_argument('--dir', dest = "save_directory", default = "averagedMultiDustSpatialDensity%d",
    ...
    parser.add_argument('--y2_range', dest = "y2_range", type = float, nargs = 2, default = [1e-4, 1e5],
...
for i in range(dust_number):
   stokes_numbers.append(fargo_par["Stokes%d" % (i)])
...
def make_plot(frame, show = False):
    ...
    normalized_gas_density = averagedGasDensity / surface_density_zero

    gas_scale_height = np.sqrt(2.0 * np.pi) * scale_height * rad
    normalized_gas_spatial_density = normalized_gas_density / gas_scale_height

    x = rad
    y = normalized_gas_spatial_density
    result,  = plot.plot(x, y, linewidth = linewidth, zorder = 99, label = "gas")
    ...
    ax2.set_ylabel(r"$\rho$ [g / cm$^3$]", fontsize = fontsize, rotation = 270, labelpad = 25)
    ...
    # Save, Show, and Close
    if version is None:
        save_fn = "%s/averagedMultiDustSpatialDensity%d_%04d.png" % (save_directory, dust_number, frame)
    else:
        save_fn = "%s/v%04d_averagedMultiDustSpatialDensity%d_%04d.png" % (save_directory, version, dust_number, frame)
