...
def new_argument_parser(description = "Plot dust density maps."):
    ...
    # Scale
    parser.add_argument('-s', dest = "scale", type = float, default = 1.0,
                         help = 'how much to scale density (default: 1)')
    ...
    parser.add_argument('--move', dest = "move", type = float, default = 0.0,
                         help = 'how far to move rings towards planet (default: 1)')


    # Files
    parser.add_argument('--dir', dest = "save_directory", default = "movedRings-dustDensityMaps%d",
    ...
    parser.add_argument('-w', dest = "weaken", action = 'store_true', default = False,
                         help = 'get rid of inner ring (default: do not weaken)')
...
def scale_density(normalized_density, fargo_par):
    """ get rid of a ring (right now, just the inner one) """
...
def weaken_density(normalized_density, fargo_par):
    ...
    weakened_density[:r1] *= 0.01

    return weakened_density
...
def make_plot(frame, show = False):
    ...
    normalized_density = scale_density(normalized_density, fargo_par)
    normalized_gas_density = scale_density(normalized_gas_density, fargo_par)
    ...
    # Write Arrays
    write_frame = frame + version + 10

    write_fn = "dust%ddens%d.dat" % (dust_number, write_frame)
