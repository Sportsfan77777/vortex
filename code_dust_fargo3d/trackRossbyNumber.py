...
def new_argument_parser(description = "Plot gas density maps."):
    ...
    # Files
    parser.add_argument('--dir', dest = "save_directory", default = "rossbyNumber",
...
def get_contrasts(args_here):
    ...
    rossby_number_over_time_98[i] = np.percentile(azimuthal_profile, 98)
    rossby_number_over_time_95[i] = np.percentile(azimuthal_profile, 95)
...
def make_plot(show = False):
    ...
    p3, = host.plot(x, y3, c = 'r', linewidth = linewidth, label = "95", zorder = 80)
    p2, = host.plot(x, y2, c = 'b', linewidth = linewidth, label = "98", zorder = 90)
    p1, = host.plot(x, y1, c = 'k', linewidth = linewidth, label = "100", zorder = 99)
    ...
    host.set_ylabel("Rossby number", fontsize = fontsize)
    ...
    if version is None:
        save_fn = "%s/%s_rossbyNumberOverTime.png" % (save_directory, directory_name)
    else:
        save_fn = "%s/v%04d_%s_rossbyNumberOverTime.png" % (save_directory, version, directory_name)
