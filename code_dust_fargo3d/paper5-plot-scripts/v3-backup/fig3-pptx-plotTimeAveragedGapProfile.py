...
def make_plot(frame, show = False):
    ...
    result,  = plot.plot(x, y, linewidth = linewidth, c = "blueviolet", zorder = 99, label = "K20")
    ...
    if args.compare is not None:
        ...
        for i, directory in enumerate(directories):
            ...
            result = plot.plot(x, y_compare, c = "slateblue", linewidth = linewidth, alpha = 0.6, zorder = 99, label = labels[i])

    # Reference 2
    colors = ['grey']
    num_jup = 3
    cases = ["inviscid-Mt%d" % num_jup] #, "inviscid-Mt%d" % num_jup]
