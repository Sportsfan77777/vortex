...
def my_model_J(r):
    ...
    if r > 1:
       ...
       b = -0.65
    flux = a * np.power(r, 0.5) + b

    if (np.abs(distance) <= 1.5 * r_hill):
      #c = a * np.power(r_p + 2.0 * r_hill, 0.5) + b
      b2 = -4.55
      flux = 5.0 * a * np.power(r, 1.0) + b2

      #b2 = -2.55
      #flux = 3.0 * a * np.power(r, 1.0) + b2

      b2 = -2.1
      flux = 2.5 * a * np.power(r, 1.0) + b2

      #flux -= 3.0 * a * np.power(r_p + 2.0 * r_hill, 1.0) + b
      #flux += c
...
def make_plot(show = False):
    ...
    plot.legend(loc = "upper left", handlelength = 2.3, fontsize = fontsize - 6)
    ...
    ax.set_ylabel(r"$J_\mathrm{MHD}$ [$10^{-3}$ $\Sigma_0 r_\mathrm{p}^3 v_\mathrm{p} \Omega_\mathrm{p}$]", fontsize = fontsize)
    ...
    ax_inset = inset_axes(ax, width="45%", height="40%", loc = "lower right", borderpad = 3)
    ...
    plot.scatter([planet_location], [y_in[0] + 1.05 * 10**(-3)], c = 'k', s = 75, alpha = 0.8, clip_on = False)

    ax_inset.set_xlim(x_in[0], x_in[1])
    ax_inset.set_ylim(y_in[0], y_in[1])

    # Save, Show, and Close
    if version is None:
        save_fn = "%s/torqueProfile.png" % (save_directory)
    else:
        save_fn = "%s/v%04d_torqueProfile.png" % (save_directory, version)
