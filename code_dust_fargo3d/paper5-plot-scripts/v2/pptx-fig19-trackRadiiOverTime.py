...
def new_argument_parser(description = "Plot gas density maps."):
    ...
    parser.add_argument('--compare', dest = "compare", nargs = '+', default = ["../h05_nu9_b3-M03-768x768-case4-stockholm-migrating", "../h05_nu9_b3-M03-768x768-case4-stockholm-migrating-no-extra-gap-torque-simple-mass-loss", "../h05_nu9_b3-M03-768x768-case12-stockholm-migrating"],
    ...
    parser.add_argument('--range', dest = "r_lim", type = float, nargs = 2, default = [0, 1500],
                         help = 'radial range in plot (default: [r_min, r_max])')
    parser.add_argument('--max_y', dest = "max_y", type = float, default = 4,
...
def make_plot(show = False):
    ...
    x = times
    y = planet_radii
    #result = plot.plot(x, y, c = "b", linewidth = linewidth, zorder = 199, label = "No Extra Gap Torque")
    ...
    colors = ["r", "turquoise", "C4"]
    labels = ["This work", "No Extra Gap Torque\n(Simple Mass Loss)", "K20"]
    labels = ["This work", "No Extra Gap Torque", "K20"]
    max_ts = [570, 1070, 1100]
    #dashes = ["-.", "-", "-", "-", "-", "-."]
    ...
    if args.compare is not None:
        for i, directory in enumerate(args.compare):
            ...
            times = data_comp[:max_t, 0] + args.offset

            planet_x = data_comp[:max_t, 1]
            planet_y = data_comp[:max_t, 2]
            planet_radii = np.sqrt(np.power(planet_x, 2) + np.power(planet_y, 2))

            x_comp = times
            y_comp = planet_radii
            result = plot.plot(x_comp, y_comp, c = colors[i], linewidth = linewidth, zorder = 99 - i, label = labels[i])

        h, l = ax.get_legend_handles_labels()
        handles_r = h[::-1]; labels_r = l[::-1]
        plot.legend(loc = "lower right", fontsize = fontsize - 5, handles = handles_r, labels = labels_r, facecolor = "white", framealpha= 0.5)
        #plot.legend(loc = "upper left", fontsize = fontsize - 5)
    ...
    text_b = r"$b = %d \times 10^{%d}$" % (wind_coefficient, wind_power)
    text_visc = r"$\nu = 0$"
    plot.text(0.05 * x_range + x_min, 0.9 * plot.ylim()[-1], text_b, fontsize = fontsize, color = 'black', horizontalalignment = 'left')
    plot.text(0.05 * x_range + x_min, 0.82 * plot.ylim()[-1], text_visc, fontsize = fontsize, color = 'black', horizontalalignment = 'left')
