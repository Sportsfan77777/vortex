...
def new_argument_parser(description = "Plot gas density maps."):
    ...
    parser.add_argument('--compare', dest = "compare", nargs = '+', default = ["../h10_nu9_b45-Mt3-768x768-case12-stockholm", "../h10_nu5_b45-Mt3-768x768-case12-stockholm", "../h10_nu5_b45-Mt3-768x768-case3-stockholm"],
    ...
    parser.add_argument('--max_y', dest = "max_y", type = float, default = 1.9,
...
def find_max(averagedDensity):
    ...
    max_density = np.max(averagedDensity[outer_disk_start:])
...
def get_max(density):
    ...
    # Get Minima
    max_density = find_max(averagedDensity)
...
def get_gap_edge_density(args_here):
    # Unwrap Args
...
peak_density_over_time = mp_array("d", len(frame_range))
...
def make_plot(show = False):
    ...
    if args.compare is not None:
        ...
        colors = ["r", "slateblue", "deepskyblue"]
        alphas = [1.0, 0.4, 0.4]
        labels = ["Two-component (K20)", "Three-component (K20)", "Four-component (K20)"]
        linewidths = [0, -2, -2]
        ...
        for i, directory in enumerate(directories):
            ...
            ### Plot ###
            x = frame_range
            y_compare = peak_density_over_time
    ...
    plot.ylabel(r"$\Sigma(r_\mathrm{bump})$ $/$ $\Sigma_0(r_\mathrm{bump})$", fontsize = fontsize)
    ...
    title1 = "Outer Pressure Bump Peak Density"
    ...
    if version is None:
        save_fn = "%s/%s_peakGapDensityOverTime.png" % (save_directory, directory_name)
    else:
        save_fn = "%s/v%04d_%s_peakGapDensityOverTime.png" % (save_directory, version, directory_name)
        save_fn = "%s/v%04d_%s_peakGapDensityOverTime.pdf" % (save_directory, version, directory_name)
    plot.savefig(save_fn, bbox_inches = 'tight', dpi = dpi, format = "pdf")
