...
def new_argument_parser(description = "Plot gas density maps."):
    ...
    parser.add_argument('--max_y', dest = "max_y", type = float, default = 1.0,
...
def get_gap_edge_density(args_here):
    ...
    peak_density = get_max(density) / surface_density_zero
...
def make_plot(show = False):
    ...
    for i, time_i in enumerate(times):
       ...
       initial_density = [np.power(r, -1.25) for r in ref_radii]
    ...
    plot.ylabel(r"$\Sigma(r_\mathrm{bump})$ $/$ $\Sigma_0$", fontsize = fontsize)
