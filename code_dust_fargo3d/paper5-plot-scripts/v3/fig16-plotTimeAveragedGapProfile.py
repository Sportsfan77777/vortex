...
def make_plot(frame, show = False):
    ...
    result,  = plot.plot(x, y, linewidth = linewidth, c = "gold", alpha = 0.9, zorder = 199, label = r"$\mathrm{b} = 10^{-3}$")
    ...
    if args.compare is not None:
        labels = [r"$\mathrm{b} = 5 \times 10^{-4}$ (fiducial)", r"$\mathrm{b} = 10^{-4}$", r"$\mathrm{b} = 10^{-5}$"]
