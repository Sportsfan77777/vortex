...
def new_argument_parser(description = "Plot gas density maps."):
    ...
    # Files
    parser.add_argument('--dir', dest = "save_directory", default = "gapDepletion",
                         help = 'save directory (default: gapDepletion)')
    ...
    parser.add_argument('--compare', dest = "compare", nargs = '+', default = ["../h10_nu9_b45-Mt3-768x768-case4-stockholm-no-extra-gap-torque", "../h10_nu5_b45-Mt3-768x768-case4-stockholm-no-extra-gap-torque"],
    ...
    parser.add_argument('--range', dest = "r_lim", type = float, nargs = 2, default = [0, 400],
                         help = 'radial range in plot (default: [r_min, r_max])')
    parser.add_argument('--max_y', dest = "max_y", type = float, default = 58,
...
def make_plot(show = False):
    ...
    x = frame_range
    y = 1.0 / gap_depth_array
    result1 = plot.plot(x, y, c = 'b', linewidth = linewidth, zorder = 199, label = "Four-component")
    ...
    for i, time_i in enumerate(times):
       ...
       ref_density_min = np.min(ref_density)
    ...
    if args.compare is not None:
        ...
        #colors = ["r", "blueviolet", "deepskyblue"]
        colors = ["firebrick", "slateblue", "deepskyblue"]
        alphas = [1.0, 0.4, 0.4]
        labels = ["Two-component", "Three-component"]
        linewidths = [0, -2, -2]
        ...
        for i, directory in enumerate(directories):
            ...
            ### Plot ###
            x = frame_range
            y_compare = 1.0 / np.array(gap_depth_over_time)
            result_compare = plot.plot(x, y_compare, c = colors[i], linewidth = linewidth + linewidths[i], alpha = alphas[i], zorder = 99, label = labels[i])
    ...
    with open("Mt%dAm3-gapDepth.csv" % (num_jupiters), "r") as f:
       ref_data = []
       reader = csv.reader(f)
       for row in reader:
          ref_data.append(row)
       ref_data = np.array(ref_data).astype(np.float)

       ref_times = ref_data[:, 0]
       ref_gap_depths = np.power(10.0, ref_data[:, 1])

       ref_result = plot.plot(ref_times, ref_gap_depths, c = 'k', linewidth = 1, zorder = 1)
       ref_result = plot.plot(ref_times, ref_gap_depths, c = 'k', linewidth = 1, zorder = 1, label = "A&B 2023")
    ...
    plot.ylabel(r"$\Sigma_\mathrm{gap}$ $/$ $\Sigma_\mathrm{p}$", fontsize = fontsize)
    ...
    if version is None:
        save_fn = "%s/%s_gapDepletionOverTime.png" % (save_directory, directory_name)
    else:
        save_fn = "%s/v%04d_%s_gapDepletionOverTime.png" % (save_directory, version, directory_name)
