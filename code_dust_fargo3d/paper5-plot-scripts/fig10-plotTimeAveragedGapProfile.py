...
def new_argument_parser(description = "Plot gas density maps."):
    ...
    parser.add_argument('--compare', dest = "compare", nargs = '+', default = ["../h10_nu5_b45-Mt5-768x768-case4-stockholm-no-mass-loss", "../h10_nu9_b45-Mt5-768x768-case12-stockholm"],
...
def make_plot(frame, show = False):
    ...
    if not args.log:
        ...
        plot.scatter([planet_location], [1.005 * 5*10**(-3)], c = 'k', s = 75, alpha = 0.8, clip_on = False)
    ...
    if args.compare is not None:
        labels = ["No Mass Loss", "Two-component (K20)"]
    ...
    for i, time_i in enumerate(times):
       ref_density = []
       with open("Mt5Am3-t%d.csv" % time_i, "r") as f:
          reader = csv.reader(f)
          for row in reader:
             ref_density.append(row)
       ref_density = np.array(ref_density).astype(np.float)
