...
def new_argument_parser(description = "Plot gas density maps."):
    ...
    parser.add_argument('--range', dest = "r_lim", type = float, nargs = 2, default = [0.3, 3],
                         help = 'radial range in plot (default: [r_min, r_max])')
    parser.add_argument('--max_y', dest = "max_y", type = float, default = 2,
    ...
    parser.add_argument('--compare', dest = "compare", nargs = '+', default = ["../h08_nu5_b0-Mt3-768x768-case4-stockholm", "../h08_nu9_b45-Mt3-768x768-case4-stockholm", "../h08_nu9_b0-Mt3-768x768-case4-stockholm"],
...
def make_plot(frame, show = False):
    ...
    if args.compare is not None:
        labels = ["Viscous w/ no wind", "Wind w/ no viscosity", "Neither"]
        colors = ["orange", "firebrick",  "grey"]
        ...
        for i, directory in enumerate(directories):
            ...
            result = plot.plot(x, y_compare, c = colors[i], linewidth = linewidth, alpha = 0.9, zorder = 99, label = labels[i])
    ...
    for i, case_i in enumerate(cases):
       ...
       yref = ref_density[:, 1]
       plot.plot(xref, yref, linewidth = linewidth, linestyle = "-.", c = colors[i % 2], zorder = 2, label = "Inviscid (t = 140)")
    ...
    for i, time_i in enumerate(times):
       ref_density = []
       with open("Mt1Am3-t%d.csv" % time_i, "r") as f:
          reader = csv.reader(f)
          for row in reader:
             ref_density.append(row)
       ref_density = np.array(ref_density).astype(np.float)
    ...
    text_h = r"$h = %.2f$" % (scale_height)
    #plot.text(-0.9 * box_size, 2, text_mass, fontsize = fontsize, color = 'black', horizontalalignment = 'left', bbox=dict(facecolor = 'white', edgecolor = 'black', pad = 10.0))
    plot.text(0.95 * x_range + x_min, 0.9 * plot.ylim()[-1], text_h, fontsize = fontsize, color = 'black', horizontalalignment = 'right')
