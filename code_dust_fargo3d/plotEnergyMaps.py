...
def make_plot(frame, show = False):
    ...
    if merge > 0:
        ...
        field = "dens"
        energy = Fields("./", 'energy', frame).get_field(field).reshape(num_rad, num_theta)
    else:
    ...
    # Save, Show, and Close
    if version is None:
        save_fn = "%s/energyMap_%04d.png" % (save_directory, frame)
    else:
        save_fn = "%s/v%04d_energyMap_%04d.png" % (save_directory, version, frame)
