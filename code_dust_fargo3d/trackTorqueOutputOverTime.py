...
def get_torque(args_here):
    ...
    def helper(density):
        ...
        px = planet_x[frame_i]
        py = planet_y[frame_i]
        planet_m = base_mass[frame_i] + accreted_mass[frame_i]
        planet_r = planet_radii[frame_i]

        hill_radius = planet_r * np.power(planet_mass, 1.0 / 3.0)
        smoothing_radius = p.thicknesssmoothing * scale_height
        if p.rochesmoothing > 0:
            smoothing_radius = p.rochesmoothing * hill_radius

        # Force
        dx = np.outer(rad, np.cos(theta - np.pi / 2.0)) - px
        dy = np.outer(rad, np.sin(theta - np.pi / 2.0)) - py

        distance_sq = np.power(dx, 2.0) + np.power(dy, 2.0) + np.power(smoothing_radius, 2.0)
        distance = np.sqrt(distance_sq)

        force_x = dx * BigG * cell_mass / np.power(distance, 3.0)
        force_y = dy * BigG * cell_mass / np.power(distance, 3.0)
