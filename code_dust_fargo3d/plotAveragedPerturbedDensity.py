...
def new_argument_parser(description = "Plot gas density maps."):
    ...
    # Files
    parser.add_argument('--dir', dest = "save_directory", default = "averagedPerturbedDensity",
                         help = 'save directory (default: averagedPerturbedDensity)')
...
def make_plot(frame, show = False):
    ...
    if args.compare is not None:
        ...
        for i, directory in enumerate(directories):
            ...
            density_zero_compare = (fromfile("%s/gasdens0.dat" % (directory)).reshape(num_rad, num_theta))

            averagedDensity_compare = np.average(density_compare, axis = 1)
            averagedDensity_zero_compare = np.average(density_zero_compare, axis = 1)

            normalized_density_compare = (averagedDensity_compare - averagedDensity_zero_compare) / averagedDensity_zero_compare  
    ...
    if args.max_y is None:
        ...
        highest_y = max(y[x_min_i : x_max_i])
        lowest_y = np.abs(min(y[x_min_i : x_max_i]))

        if highest_y > lowest_y:
            max_y = highest_y
        else:
            max_y = lowest_y
    ...
    ax.set_ylabel(r"$\Delta \Sigma$ $/$ $\Sigma_0$", fontsize = fontsize)
    ...
    ax.text(x_mid, y_text * (plot.ylim()[-1] - plot.ylim()[0]) + plot.ylim()[0], title1, horizontalalignment = 'center', bbox = dict(facecolor = 'none', edgecolor = 'black', linewidth = 1.5, pad = 7.0), fontsize = fontsize + 2)
    ...
    # Save, Show, and Close
    if version is None:
        save_fn = "%s/averagedPerturbedDensity_%04d.png" % (save_directory, frame)
    else:
        save_fn = "%s/v%04d_averagedPerturbedDensity_%04d.png" % (save_directory, version, frame)
