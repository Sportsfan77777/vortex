...
def new_argument_parser(description = "Plot gas density maps."):
    ...
    # Files
    parser.add_argument('--dir', dest = "save_directory", default = "azimuthalVariance",
                         help = 'save directory (default: azimuthalVariance)')
...
def make_plot(frame, show = False):
    ...
    normalized_density = np.average(density, axis = 1) / surface_density_zero

    azimuthalVariance = np.std(density, axis = 1)
    normalized_variance = azimuthalVariance / normalized_density
    ...
    # Save, Show, and Close
    if version is None:
        save_fn = "%s/azimuthalVariance_%04d.png" % (save_directory, frame)
    else:
        save_fn = "%s/v%04d_azimuthalVariance_%04d.png" % (save_directory, version, frame)
